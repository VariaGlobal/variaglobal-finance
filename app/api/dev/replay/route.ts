...
export async function GET() {
  ...
  const checks: Check[] = [
    ...
    check(
      'JULY-PAYABLE',
      'July 1–15 payable base matches sheet E12',
      cents(sheets.july.pivotTotalE12),
      july.payableCents,
      'Resolved, non-routed, non-pending lines only. The deferred-in Arsalan line is excluded until RUL-002 is decided.',
    ),
    check(
      'JULY-VENDOR',
    ...
    check(
      'JULY-RULING-FULL',
      'Jul 31 total if RUL-002 = pay full',
      cents(sheets.july.pivotTotalE12) + cents(sheets.july.deferredInFull),
      july.pendingRulingTotals.find((t) => t.label.includes('full'))?.totalCents ?? -1,
      'Payable base + $5,150.',
      0,
    ),
    check(
      'JULY-RULING-MINUS3H',
      'Jul 31 total if RUL-002 = apply −3h via IM',
      cents(sheets.july.pivotTotalE12) + cents(sheets.july.deferredInMinus3h),
      july.pendingRulingTotals.find((t) => t.label.includes('−3h'))?.totalCents ?? -1,
      'Payable base + $4,850 (48.5h × $100); the 3h moves to Interrupt Media settlement.',
      0,
    ),
  ]
